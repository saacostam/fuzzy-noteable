...
export class UserAuthService {
    ...
    async loginUser(credentials: {username: string, password: string}): Promise<AuthTokensDto>{
        const matchingUserWithPassword = await this.userService.getByUsername({username: credentials.username}, {includePassword: true});
    ...
    async handleUserRefreshTokenProcess(refreshToken: string): Promise<AuthTokensDto>{
        if (!this.authService.isValidRefreshToken(refreshToken)) throw new UnauthorizedException(UNAUTHORIZED_ERROR_MESSAGE.INVALID_REFRESH_TOKEN)

        const prevTokenPayload = await this.authService.getRefreshTokenContent(refreshToken);

        if (!prevTokenPayload || !prevTokenPayload.username) throw new UnauthorizedException(UNAUTHORIZED_ERROR_MESSAGE.INVALID_REFRESH_TOKEN)

        const user = await this.userService.getByUsername({ username: prevTokenPayload.username}, {includePassword: false});

        if (!user) throw new UnauthorizedException(UNAUTHORIZED_ERROR_MESSAGE.INVALID_REFRESH_TOKEN)
