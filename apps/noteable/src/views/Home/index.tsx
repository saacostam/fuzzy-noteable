    ...
    isLoading?
      <>
        <div className={'flex flex-row mb-4'}>
          <Skeleton variant={'rounded'} height={60} width={60} className={'mr-3'}/>
      ...
      (isSuccess && tablature) ?
        <>
          <div className={'mb-4 flex flex-row'} >
            <div className={'bg-blue-500 w-[60px] h-[60px] flex items-center justify-center rounded-lg mr-3'}>
              <LibraryMusicRounded className={'text-white'}/>
            </div>
            <div className={'flex flex-col'}>
              <h3 className={'text-2xl'}>{tablature.song.name}</h3>
              <a href={`/artist/${tablature.song.artists[0].id}`}><h6 className={'text-sm text-blue-500 font-semibold underline'}>{tablature.song.artists[0].name}</h6></a>
            </div>
          </div>
