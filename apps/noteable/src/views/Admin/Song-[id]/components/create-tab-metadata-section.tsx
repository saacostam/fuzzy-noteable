...
type CommonChangeInputEventType = ChangeEvent<HTMLTextAreaElement | HTMLInputElement>;
...
export function CreateTabMetaDataSection({
  ...
  const handleDifficultyOnChange = useCallback(
    (e: SelectChangeEvent<string>) => (handleChangeDifficulty((e.target.value as Difficulty))),
    [handleChangeDifficulty]
  );
  ...
  return (
    <>
      <Typography variant={'h5'} className={'text-center'} sx={{mb:2}}>Tab Meta Data</Typography>
