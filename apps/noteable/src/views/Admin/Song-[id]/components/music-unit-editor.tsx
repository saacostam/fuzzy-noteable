import {AllChordSuffixes, ChordName, ChordSuffix, MusicUnitChordWithId, Note, SortedNotes} from "@noteable/types";
...
export function MusicUnitEditor({
  ...
  const handleNoteOnChange = useCallback((e: SelectChangeEvent) => {
  ...
  const handleSuffixOnChange = useCallback((e: SelectChangeEvent) => {
    const suffix = e.target.value as ChordSuffix;
    const {note} = parseChord(musicUnit.self);
    handleMusicUnitChange(musicUnit.id, `${note}${suffix}`, musicUnit.dur);
  }, [handleMusicUnitChange, musicUnit]);

  const handleDurationOnChange = useCallback((e: ChangeEvent<HTMLTextAreaElement | HTMLInputElement>) => {
        ...
        >
          {
            AllChordSuffixes.map(chordSuffix => (
              <MenuItem value={chordSuffix} key={chordSuffix}>{chordSuffix}</MenuItem>
            ))
          }
        </Select>
        <TextField
