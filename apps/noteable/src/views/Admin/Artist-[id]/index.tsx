...
export function CreateSongSection(
  ...
  return (
    <Box sx={{p: 2}}>
      <Typography variant={'h5'} className={'text-center'} sx={{mb:2}}>Admin: Create Song</Typography>
      <TextField
        size={"small"}
        label={"Song name"}
        ...
        >
          {
            scaleTypes.map(scaleType => (
              <MenuItem value={scaleType} key={scaleType}>{scaleType}</MenuItem>
            ))
          }
        </Select>
      </Stack>
      ...
      >
        {
          allDecades.map(decade => (
            <MenuItem value={decade} key={decade}>{decade}</MenuItem>
          ))
        }
      </Select>
      <Select
...
function ArtistSongSection({ isArtistSongsReady, artistName, artistSongs }: ArtistSongsSectionProps){
  return (
    isArtistSongsReady ? (
      <>
