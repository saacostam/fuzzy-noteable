...
export function CreateArtistSection({ registerCreateArtistForm, doCreateArtist, createArtistIsLoading}: CreateArtistSectionProps){
  return (
    <Box sx={{p: 2}}>
      <Typography variant={'h5'} className={'text-center'} sx={{mb:2}}>Admin: Create Artist</Typography>
...
export function ArtistsSection({artists}: ArtistsSectionProps){
  return (
    <Box sx={{p: 2}}>
      <Typography variant={'h5'} className={'text-center'} sx={{mb:1}}>Admin: Artists</Typography>
      {
        isEmpty(artists) ? (
          <>
            <Typography variant={'h6'} color={'error'}>Empty Response</Typography>
            <Typography>There are no artists in the database. Please create new artists or check the db/query for possible errors.</Typography>
          </>
        ) : (
          <List
            dense={true}
