...
export class DomHandler {
  ...
  buildErrorMessageWhenFailingToFindElementBySelector = (element: string) =>
    `No ${element} element was found for the given ID. Please check if the provided ID is correct or exclude it to create a(n) ${element} element from scratch.`;
  ...
  constructor({ canvasId, playerId }: DomHandlerParameters) {
    ...
    if (canvasId) {
      ...
      if (!element || !(element instanceof HTMLCanvasElement))
        throw new DomSetupError(
          this.buildErrorMessageWhenFailingToFindElementBySelector('canvas')
        );
      maybeCanvas = element;
    }
    ...
    if (playerId) {
      ...
      if (!element)
        throw new DomSetupError(
          this.buildErrorMessageWhenFailingToFindElementBySelector('player')
        );
      maybePlayer = element;
    }
    ...
    this.playerElement = maybePlayer
      ? maybePlayer
