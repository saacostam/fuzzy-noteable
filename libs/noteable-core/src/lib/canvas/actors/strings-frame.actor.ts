...
export class StringsFrameActor extends Actor{
    ...
    async update(handler: CanvasHandler) {
        ...
        this.musicUnitQueue = handler.dataHandler.getRangeWithSlack(this.currentTime*1000, StringsFrameActor.SYNC_DELTA_TIME);
    }
    ...
    draw(ctx: CanvasRenderingContext2D) {
        ...
        for (let string = 0; string < 6; string++) this.drawLine(ctx, this.x, this.y + (DELTA_Y * string), this.x + this.width, this.y + (DELTA_Y * string));
        ...
        ctx.fillRect(0, this.y - CHORD_PREVIEW_SLACK, BASE_X, this.height + (2 * CHORD_PREVIEW_SLACK));
        ...
        ctx.fillRect(this.x + this.width, this.y - CHORD_PREVIEW_SLACK, CANVAS_WIDTH - (this.x + this.width), this.height + (2 * CHORD_PREVIEW_SLACK));

        // Current Time Indicator
        ctx.lineWidth = 0.0112 * CANVAS_HEIGHT;
        ctx.strokeStyle = 'rgba(218,44,56,0.7)';
        ctx.lineCap = 'round';
        this.drawLine(ctx, BASE_X, this.y - CURRENT_TIME_INDICATOR_SLACK, BASE_X, this.y + this.height + CURRENT_TIME_INDICATOR_SLACK);
    }

    private drawChords(ctx: CanvasRenderingContext2D, BASE_X: number, CHORD_PREVIEW_SLACK: number){
        const CANVAS_WIDTH =  ctx.canvas.width;
        ...
        for (let i = 0; i < this.musicUnitQueue.length; i++){
            ...
            const X_PROPORTION = (leftTime - (this.currentTime * 1000))/StringsFrameActor.SYNC_DELTA_TIME;
            const WIDTH_PROPORTION = (rightTime - leftTime)/StringsFrameActor.SYNC_DELTA_TIME;
