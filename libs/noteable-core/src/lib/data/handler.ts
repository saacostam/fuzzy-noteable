...
export class DataHandler {
  ...
  buildSchedule(tablature: Tablature): SyncMusicUnit[] {
    ...
    if (!musicUnits[0].bpm || !musicUnits[0].syncPnt || !musicUnits[0].timeSig)
      throw new DataError(
    ...
    for (const mUnit of musicUnits) {
      ...
      if (mUnit.type === 'ch') {
        const chord = mUnit.self.includes(SILENCE_CHORD_NAME)
  ...
  getCurrentAndNext(currentTime: number) {
    ...
    return {
      ...
      next:
        index + 1 < this.schedule.length ? this.schedule[index + 1] : undefined,
    };
  }
  ...
  private transpose(up?: boolean) {
    ...
    this.tablature.transposition = ((this.tablature.transposition +
      ...
      notesToTranspose.length) as Transposition;
    this.schedule.forEach((SMU) => {
      if (SMU.type === 'ch') {
        ...
        const newNoteIndex =
          (notesToTranspose.findIndex((val) => val === note) +
            (!up ? 1 : -1) +
            notesToTranspose.length) %
          notesToTranspose.length;
